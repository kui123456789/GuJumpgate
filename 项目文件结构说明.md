...
- `phone-sms/providers/registry.js`：手机接码 provider 注册表，统一维护 provider 标识、展示名称、模块 key、默认平台与 provider 顺序归一化。
- `phone-sms/providers/hero-sms.js`：HeroSMS 接码平台适配层，负责 HeroSMS 国家、价格、余额与基础 provider 能力归一化。
- `phone-sms/providers/five-sim.js`：5sim 接码平台适配层，负责 5sim 国家、运营商、价格、购买、复用、轮询、完成、取消与 ban 接口封装。
- `phone-sms/providers/smsbower.js`：SMSBower 接码平台适配层，负责官方 `handler_api.php` 的余额、价格、取号、查码、完成、取消与 ban 封装；OpenAI 服务码默认 `dr`，美国 `+1` 号码提交时归一为本地 10 位号。
- `phone-sms/providers/hosted-sms.js`：托管短信接口适配层，负责解析 `phone----url` 号码池、固定美国本地号提交、接口防缓存轮询、验证码提取、最少使用次数选号，以及 `hostedSmsPoolUsage / hostedSmsCurrentEntry` 运行记录更新。
...
- `sidepanel/sidepanel.html`：侧边栏页面结构；当前步骤列表已改为动态容器，日志区提供“记录”按钮并挂接账号记录覆盖层，顶部新增“贡
  ...
  展示 `OAUTH / 回调 / 总状态` 三块真实运行态信息，同时把“接码”开关与“验证码重发”拆成同一行展示；只有开启接码后，下面的接码平台、
  国家、API Key 或托管短信接口号码池配置才会显示；页面继续加载 `managed-alias-utils.js` 与 `mail-provider-utils.js`，并把旧的“邮箱前缀”字段语义改为“别
...
- `sidepanel/sidepanel.js`：侧边栏主入口脚本，负责 UI 状态同步、动态步骤渲染、按钮交互、共享验证码自动重发次数配置与广播接收，并装
  ...
  会在 sidepanel 初始化时拉取并恢复到本地保存的国家/API 设置；托管短信接口会保存独立号码池 `hostedSmsPoolText`，不复用 PayPal hosted checkout 号码池；接码开关关闭时会直接隐藏相关配置；账号记录快照同步改为默认自动模式，只
...
- `tests/hosted-sms-provider.test.js`：测试托管短信接口号码池解析、美国本地号归一、验证码提取、元数据数字忽略、最少使用次数选号和 Step9 换号时跳过已试号码。
