...
5. SUB2API 多分组创建账号。
6. OAuth 登录使用全新标签页。
7. 接码平台多平台适配：HeroSMS + 5sim + SMSBower。
...
补充：接码平台多平台适配是当前分支新增的本地自定义功能；真实 5sim / SMSBower API Key 只允许通过侧边栏输入并保存到本地配置，不写入仓库、测试或文档。
...
## 8. 接码平台多平台适配：HeroSMS + 5sim + SMSBower
...
- 新增 `SMSBower` 平台，只用于 Auth/OAuth 后置手机号验证 Step 9；OpenAI 服务码默认 `dr`，内置美国 `187`、马来西亚 `7`、加纳 `38`、泰国 `52`、印度尼西亚 `6`、越南 `10`。
- 国家/地区列表、API Key、价格上限、余额、买号、查码、完成、取消、ban、复用均按当前 `phoneSmsProvider` 分发。
- HeroSMS 继续使用原 `heroSms*` 字段；5sim 使用独立 `fiveSim*` 字段；SMSBower 使用独立 `smsBower*` 字段，切换平台时不会互相覆盖草稿。
...
| `phone-sms/providers/smsbower.js` | SMSBower 适配层，封装官方 `handler_api.php` 的 balance、price、getNumber、getStatus、setStatus，并处理美国 `+1` 本地号提交。 |
...
| `sidepanel/sidepanel.html` | 接码平台下拉、余额按钮、5sim operator 输入、SMSBower API Key / 国家优先级 / 服务代码输入。 |
| `sidepanel/sidepanel.js` | 平台切换、地区列表联动、价格/余额查询、配置收集与恢复。 |
| `tests/five-sim-provider.test.js` | 5sim provider 单测。 |
| `tests/smsbower-provider.test.js` | SMSBower provider 单测。 |
...
- 通用：
  - `phoneSmsProvider: "hero-sms" | "5sim" | "nexsms" | "smsbower" | "hosted-sms"`
- HeroSMS：
  - `heroSmsApiKey`
  - `heroSmsCountryId`
  - `heroSmsCountryLabel`
...
1. 不要把真实 5sim / SMSBower key 写入代码、测试或文档；只允许用户在侧边栏输入。
2. 上游同步时如果改了 Step 9 手机号验证，必须保留 `currentPhoneActivation.provider` 驱动的后续分发。
3. 上游同步时如果改了 sidepanel 接码区域，必须保留 `select-phone-sms-provider`，且地区列表按平台加载：HeroSMS 数字国家 ID，5sim 字符串 country slug，SMSBower 官方国家 ID。
...
```bash
rg -n "phoneSmsProvider|select-phone-sms-provider|PhoneSmsFiveSimProvider|PhoneSmsBowerProvider|PhoneSmsProviderRegistry|fiveSim|smsBower" \
  background.js \
