...
本教程用于说明相关项目地址、扩展更新方式、`Cloudflare Temp Email`、`iCloud 隐私邮箱` 与 `QQ 邮箱` 的使用方法、多平台手机接码扩展能力、`PayPal` 注册绑卡流程、扩展内置动态 IP 代理、节点检测与纯净度检查，以及 [Clash Verge](https://github.com/clash-verge-rev/clash-verge-rev) 的 `🔁 非港轮询` 配置方法。
...
- 需要在 `Step 9` 的手机号验证阶段使用 HeroSMS / 5sim / NexSMS / SMSBower / 托管短信接口接码（含多国家回退与价格策略）
- 需要注册并使用 `PayPal` 个人账户
...
- 一个可正常接收短信的手机号
- 一个可用的接码平台 API Key（如 HeroSMS / 5sim / NexSMS / SMSBower，用于手机号接码）
- 一张可在线支付的借记卡或信用卡
...
3. 展开设置后，按需要选择 `HeroSMS`、`5sim`、`NexSMS`、`SMSBower` 或“托管短信接口”；默认仍是 `HeroSMS / OpenAI`。

#### 二、国家与优先级（新增能力）

1. 在 `接码国家` 中多选国家，按你的业务顺序排列；SMSBower 内置美国 `+1 (187)`、马来西亚 `+60 (7)`、加纳 `+233 (38)`、泰国 `+66 (52)` 等国家，服务代码默认 `dr`。
2. 在 `国家优先级` 中选择策略：
   - `国家优先`：严格按你选择的顺序先后尝试。
