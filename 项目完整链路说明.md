...
- 展示一个单独的“接码”开关、注册方式 `signupMethod` 与“接码平台”下拉；接码平台当前支持 HeroSMS / 5sim / NexSMS / 托管短信接口。普通模式下开启接码后可把注册方式切到手机号注册，并在 OAuth 登录链路命中手机号登录验证码页时继续复用同一手机号续跑短信验证；托管短信接口只用于邮箱注册后 OAuth Step9 的 `add-phone / phone-verification` 后置手机号验证
...
- 注册方式 `signupMethod`
- 接码开关、接码平台 `phoneSmsProvider`，以及 HeroSMS / 5sim / NexSMS 各自的 API Key、国家/地区、价格上限和平台扩展设置；托管短信接口额外保存独立号码池 `hostedSmsPoolText` 和使用记录 `hostedSmsPoolUsage`，不复用 PayPal hosted checkout 号码池
- iCloud 相关偏好
- LuckMail API 配置
- 自动运行默认配置
...
3. OpenAI 手机号验证流程会按当前 sidepanel 中保存的接码平台 `phoneSmsProvider` 选择 HeroSMS / 5sim / NexSMS / 托管短信接口，并使用对应平台的 API Key、国家/地区、价格上限或托管号码池申请号码；托管短信接口的号码格式为 `2092905100----https://example.test/api/sms/recordText?key=replace-me`，10 位号码固定按美国 `+1` 本地号提交
4. 后台提交号码后，会按 `currentPhoneActivation.provider` 分发后续查码、完成、取消、ban、reuse：HeroSMS 走原 `getStatus / setStatus` 逻辑，5sim 走 `/v1/user/check|finish|cancel|ban|reuse`，NexSMS 走消息接口，托管短信接口只轮询配置的 URL 并追加 `t=Date.now()` 防缓存，完成/取消/封禁只更新本地使用记录
...
- HeroSMS / 5sim / NexSMS / 托管短信接口都归入 OpenAI 接码平台适配层；默认平台仍是 HeroSMS，5sim 产品码固定为 `openai`，operator 默认 `any`。托管短信接口只处理 Auth/OAuth 后链 Step9 的后置手机号验证，不参与手机号注册、手机号登录验证码或 PayPal hosted checkout。该适配层暂不进入多注册 flow 的 core 通用服务，后续只有第二个真实 flow 也需要接码时再按实际复用点抽象。
- 托管短信接口按号码池最少使用次数选择下一条；如果当前号码被 OpenAI 拒绝、验证码超时或接口没有有效验证码，会优先跳过本轮已试号码并切换号码池下一条，没有可替换号码时直接明确失败。
